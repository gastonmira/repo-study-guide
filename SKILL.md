...
- **Guide exists** → extract the commit it was generated from:
  1. Primary: `grep -o 'repo-study-guide-commit" content="[^"]*"' <output_dir>/study_guide.html`.
  2. Fallback (guides generated by older template versions): parse the hash from the visible `commit <code>...</code>` text in the header.
...
- If there are changes → **ask the user** which mode to use: incremental update or full regeneration. Phrase it with the previous guide's commit, its generation date, and the number of changed files. Do not pick a mode silently. If the user already stated a mode in their request (e.g. "update the guide" or "regenerate from scratch"), skip the question and honor it — this also covers non-interactive runs.
